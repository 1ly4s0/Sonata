...
export default function FolderDetailScreen() {
  ...
  const { folders } = useSelector((s: RootState) => s.library);
  ...
  const handlePlay = useCallback(async (song: Song) => {
    ...
    await playSong(folder.songs, idx >= 0 ? idx : 0);
    dispatch(addToRecentlyPlayed(song.id));
    navigation.navigate('NowPlaying');
  }, [folder, dispatch, navigation]);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle} numberOfLines={1}>{folder?.name || t('common_folder_fallback')}</Text>
        <View style={{ width: 24 }} />
      </View>
      <FlatList
        data={folder?.songs || []}
