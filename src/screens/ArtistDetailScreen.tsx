...
export default function ArtistDetailScreen() {
  ...
  const { artists } = useSelector((s: RootState) => s.library);
  ...
  const handlePlay = useCallback(async (song: Song) => {
    if (!artist) return;
    const idx = artist.songs.indexOf(song);
    await playSong(artist.songs, idx >= 0 ? idx : 0);
    dispatch(addToRecentlyPlayed(song.id));
    navigation.navigate('NowPlaying');
  }, [artist, dispatch, navigation]);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle} numberOfLines={1}>{artist?.name || t('common_artist_fallback')}</Text>
        <View style={{ width: 24 }} />
      </View>
      <FlatList
        data={artist?.songs || []}
